...
G.SCENES.dagongChoice = function (b, done) {
  ...
  const C2 = C.dagong.choice;
  G.show(
    '<div class="cardbox"><span class="app-chip">' + C2.app + '</span>' +
    '<div class="card-title">' + C2.title + '</div>' +
    '<div class="card-who">来自：' + C2.who + '</div>' +
    '<div class="card-text">' + C2.text + '</div>' +
    '<div class="choices"><button class="btn" id="dg-go">跟表哥走（跳过高考）</button>' +
    '<button class="btn primary" id="dg-stay">回去备考</button></div></div>', {});
  G.$('#dg-go').onclick = () => {
    S.flags.dagongChosen = 1; S.flags.dagong = 1;
    G.applyChoice(C2.ch[0]);
    // 跳过高考→大学→求职→职场→相亲，直接插打工支线+终幕
    const skipIds = ['exam','province','score','zhiyuan','kzq','gradPath','hrA','hrB','jargon','work','layoffTalk','track3','trackGate'];
    S.Q = S.Q.filter(b => !skipIds.includes(b.id));
    // 移除大学/求职/职场/相亲的全部节拍（第 二 幕 到 终 幕 之前）
    const uniStart = S.Q.findIndex(b => b.lines && b.lines[0] && b.lines[0].t === '第 二 幕');
    const finStart = S.Q.findIndex(b => b.lines && b.lines[0] && b.lines[0].t === '终 幕');
    if (uniStart >= 0 && finStart >= 0) S.Q.splice(uniStart, finStart - uniStart);
    // 插入打工支线
    const norm = G.norm;
    const finIdx = S.Q.findIndex(b => b.lines && b.lines[0] && b.lines[0].t === '终 幕');
    const beats = C.dagong.beats.map(c => norm({ type: 'card', app: c.app, who: c.who, title: c.title, text: c.text, ch: c.ch }));
    beats.push(norm({ type: 'card', app: C.dagong.final.app, who: C.dagong.final.who, title: C.dagong.final.title, text: C.dagong.final.text, ch: C.dagong.final.ch }));
    if (finIdx >= 0) S.Q.splice(finIdx, 0, ...beats);
    else S.Q.push(...beats);
    G.save(); done();
  };
  G.$('#dg-stay').onclick = () => { done(); };
};

/* ---------- v1.17：反事实回响 ---------- */
G.SCENES.counterfactual = function (b, done) {
  const S = G.S;
  const matched = (C.counterfactuals || []).filter(c => { try { return c.cond(S); } catch (e) { return false; } });
  const picked = [];
  const pool = matched.slice();
  while (picked.length < Math.min(2, pool.length)) {
    const k = Math.floor(G.rng() * pool.length);
    picked.push(pool.splice(k, 1)[0]);
  }
  const items = picked.map(c => '<div class="echo-card" style="border-left-color:var(--gold)"><p>' + c.text + '</p></div>').join('');
  const fallback = '<div class="echo-card" style="border-left-color:var(--gold)"><p>你的人生里有很多个如果。每一个都通向一个不同的你。</p></div>';
  G.show(
    '<div style="max-width:360px;margin:0 auto">' +
...
G.SCENES.share = function (b, done) {
  ...
  // 皮肤循环
  const skins = ['paper', 'obituary', 'doc'];
  const skinNames = { paper: '纸面', obituary: '讣告', doc: '红头文件' };
  let skinIdx = 0;
  const skinBtn = document.createElement('button');
  skinBtn.className = 'btn small center';
  skinBtn.style.marginTop = '6px';
  skinBtn.textContent = '皮肤：' + skinNames[skins[0]] + ' ▸';
  G.$('#shareBox').after(skinBtn);
  skinBtn.onclick = () => {
    skinIdx = (skinIdx + 1) % skins.length;
    skinBtn.textContent = '皮肤：' + skinNames[skins[skinIdx]] + ' ▸';
    drawShare(cv, { e, q, S, pc }, skins[skinIdx]);
  };
  drawShare(cv, { e, q, S, pc }, skins[0]);
  G.$('#sh-dl').onclick = () => {
